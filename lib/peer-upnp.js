	...
	var stub = function(inputs, callback){
		...
		if (typeof callback == "function") {
			httpRequest(opt, soap, function(err, xml) {
				...
				xml2js.parseString(xml,{ tagNameProcessors: [processors.stripPrefix], mergeAttrs: true, explicitArray: false, ignoreXmlns: true, ignoreAttrs: true},function(err,json){
					var result;
					if (err) {
...
RemoteDevice.prototype.bind = function(callback){
	...
	httpRequest(this.descriptionUrl, function(err, data) {
		...
		xml2js.parseString(data, { tagNameProcessors: [processors.stripPrefix], explicitArray: false, ignoreXmlns: true, mergeAttrs: true},function (err, json) {
			if (err || !json) return;
			var options = json.root.device;
...
var handlePostControl = function(req,rsp,peer){
	...
	if (split.length==2) {
		...
		if (service && actionName) {
			xml2js.parseString(req.data,{ tagNameProcessors: [processors.stripPrefix], mergeAttrs: true, explicitArray: false, ignoreXmlns: true, ignoreAttrs: true},function(err,json){
				if (err) {
					rsp.statusCode = 400;
...
var handlePostEvent = function(req,rsp,peer){
	...
	if (split.length==2) {
		...
		var service = device && device.services[serviceType];
		if (service) {
			xml2js.parseString(req.data,{ tagNameProcessors: [processors.stripPrefix], mergeAttrs: true, explicitArray: false, ignoreXmlns: true},function(err,json){
				if (json) {
					var data = {};
